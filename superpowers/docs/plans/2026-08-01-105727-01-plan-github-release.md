...
**Acceptance:** 推送前确认工作区内容已理解并提交、`master` 不落后于 `origin/master`、目标 Tag 在本地和远端均不存在、验证通过；使用原子推送同步 `master` 与 Tag，推送后确认分支提交与 Tag 已存在于 `origin`。若推送失败且 Tag 仅存在于本地，删除本次创建的本地 Tag；禁止覆盖 Tag、强推、拉取、变基或合并。

### Outcome 1: 发布输入确定
- Work: 审查工作区和 `v4.0.1..HEAD` 的全部非 Merge 提交，按最高语义等级计算唯一的新版本。
- Verify: `git status --short --branch && git log v4.0.1..HEAD --no-merges --pretty=fuller`
