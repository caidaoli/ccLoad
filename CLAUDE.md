...
- **Codex 上游 Header 契约**(`codex_credentials.go`+`codex_upstream_websocket.go`):不走通用反代透传;HTTP 只接收 Codex 客户端白名单,静态 Key 与 OAuth 都只用 `Authorization: Bearer`;仅当 `User-Agent` 能识别为官方 Codex 客户端且同时带非空 `Version` 时保留整组真实客户端身份,不完整或任意第三方值都回退内建官方指纹,`Originator` 固定重建。HTTP 与原生 WebSocket 都保留规范 `Session-Id`+`Thread-Id`;WebSocket 额外接收 turn state/timing/`OpenAI-Beta`,握手前删除 HTTP 传输头并归一 `OpenAI-Beta`,同时从规范会话值同步旧 `Session_id`/`Conversation_id` 别名;渠道自定义 Header 规则仍可显式增加非认证 Header
...
- **OAuth 配额采样完整性**:Anthropic 被动响应和 xAI 周/月端点部分失败走 `ReconcilePartial`,保留未返回窗口;完整快照也不得删除采样时间比它更新的窗口。退休截止取有效窗口 `SampledAt` 与请求完成时间的最早值,避免 Codex reset credits/Anthropic profile 请求延迟掩盖旧额度采样。Codex 展示合并还必须核对同 key 的时长和周期边界,不匹配时保留官方值
