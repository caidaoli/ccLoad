...
- **多 URL**:探索优先 → 1/EWMA 加权随机,失败 URL 独立退避;延迟数据只来自真实请求的 TTFB。`ChannelURL.Exact` 派生运行时 `#` 标记实现精确转发,持久化 URL 本身不含标记。Antigravity OAuth 例外:固定 provider 回退顺序(daily → production → daily sandbox),不被延迟重排,仅跳过手动禁用的 URL(`orderURLsInConfiguredOrder`)
