...
func (s *Server) selectCandidatesByChannelType(ctx context.Context, channelType string) ([]*model.Config, error) {
...
// 性能优化：冷却过滤已在 SQL 层面完成（LEFT JOIN cooldowns），无需额外循环
func (s *Server) selectCandidates(ctx context.Context, model string) ([]*model.Config, error) {
	// 直接从数据库查询（已过滤冷却渠道，按优先级排序）
