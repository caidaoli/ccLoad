...
async function editChannel(id) {
  ...
  editingChannelId = id;
  editingChannelAuthType = ['codex_oauth', 'antigravity_oauth', 'xai_oauth', 'anthropic_oauth'].includes(channel.auth_type)
    ? channel.auth_type
    : 'api_key';
  clearChannelDuplicateHint();
...
async function saveChannel(event) {
  ...
  const isOAuth = ['codex_oauth', 'antigravity_oauth', 'xai_oauth', 'anthropic_oauth'].includes(editingChannelAuthType);
...
async function fetchModelsFromAPI() {
  let endpoint;
  let fetchOptions;
  if (['antigravity_oauth', 'codex_oauth', 'xai_oauth', 'anthropic_oauth'].includes(editingChannelAuthType)) {
    if (!editingChannelId) {
