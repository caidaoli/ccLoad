...
test('channels 页底部分页跳转输入框使用主题变量适配暗色模式', () => {
  const inputRuleMatch = channelsCss.match(/\.logs-pagination-card\s+\.logs-jump-input\s*\{[^}]+\}/);
  assert.ok(inputRuleMatch, '缺少 channels 分页跳转输入框样式');
  assert.match(inputRuleMatch[0], /background:\s*var\(--field-bg\)/);
  assert.match(inputRuleMatch[0], /color:\s*var\(--neutral-900\)/);
  assert.doesNotMatch(inputRuleMatch[0], /background:\s*(?:white|rgba\(255,\s*255,\s*255,\s*0\.9\))/);
  assert.doesNotMatch(inputRuleMatch[0], /color-scheme:\s*light/);

  const focusRuleMatch = channelsCss.match(/\.logs-pagination-card\s+\.logs-jump-input:focus\s*\{[^}]+\}/);
  assert.ok(focusRuleMatch, '缺少 channels 分页跳转输入框 focus 样式');
  assert.match(focusRuleMatch[0], /background:\s*var\(--field-bg-focus\)/);
  assert.doesNotMatch(focusRuleMatch[0], /background:\s*white/);
});
