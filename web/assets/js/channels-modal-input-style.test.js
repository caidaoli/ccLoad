...
test('编辑弹窗默认数据使用紧凑密度，2URL2Key6模型不应撑出内容区滚动', () => {
  ...
  assert.match(css, /#channelModal\s+\.inline-table-container\.tall\s*\{[\s\S]*?min-height:\s*0;[\s\S]*?max-height:\s*260px;/);
  assert.match(css, /#channelModal\s+\.inline-table\s+th\s*\{[\s\S]*?padding:\s*4px\s+8px;/);
