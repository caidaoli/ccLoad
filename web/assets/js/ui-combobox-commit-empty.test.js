...
test('createSearchableCombobox 支持选项级 className 透传到下拉项', () => {
  assert.match(uiSource, /返回 \[\{value,\s*label,\s*className\?\}\]/);
  assert.match(
    uiSource,
    /if\s*\(item\.className\)\s*\{[\s\S]*?row\.classList\.add\(\.\.\.String\(item\.className\)[\s\S]*?split\([^)]+\)[\s\S]*?filter\(Boolean\)\);[\s\S]*?\}/
  );
});
