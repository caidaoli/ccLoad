...
test('tokens 编辑令牌弹窗按基础信息、配额信息、模型限制三段组织', () => {
  ...
  assert.match(tokensHtml, /class="form-row-inline__content token-limit-control token-edit-cost-control"[\s\S]*?class="token-limit-input-line token-edit-cost-row"[\s\S]*?id="editCostLimitUSD"[\s\S]*?class="token-limit-meta"[\s\S]*?id="editCostUsedDisplay"/);
  assert.match(tokensCss, /\.token-limit-control\s*\{[\s\S]*?flex-direction:\s*column;[\s\S]*?gap:\s*5px;/);
  assert.match(tokensCss, /\.token-limit-input-line\s*\{[\s\S]*?display:\s*flex;[\s\S]*?width:\s*100%;/);
