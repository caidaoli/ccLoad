...
test('共享筛选状态模块支持 URL 优先、本地存储兜底和别名查询', () => {
  ...
  const restored = window.FilterState.restore({
    search: '?channel_name=from-url&model_like=gpt-5',
    savedFilters: {
      range: 'last_month',
...
test('共享筛选状态模块可以按字段定义构建查询参数', () => {
  ...
  const fields = [
    ...
    { key: 'channelName', queryKeys: ['channel_name_like', 'channel_name'] },
    { key: 'model', queryKeys: ['model_like', 'model'] },
    {
      key: 'channelType',
...
test('共享筛选状态模块可以在保留无关参数的同时合并并清理筛选别名参数', () => {
  ...
  assert.equal(params.get('range'), 'today');
  assert.equal(params.get('channel_name_like'), 'new-name');
  assert.equal(params.has('channel_name'), false);
...
test('共享筛选状态模块可以为无 URL 参数场景构建保存筛选条件回填 search', () => {
  ...
  assert.equal(
    ...
    '?range=today&channel_name_like=saved-name&model_like=gpt-5'
  );
...
test('共享筛选状态模块可以构建 merge/replace 两种 URL 写回结果', () => {
  ...
  assert.equal(
    ...
    '?page=2&keep=1&range=today&channel_name_like=new-name'
  );
...
test('共享筛选状态模块可以按指定 history 方法写回 URL', () => {
  ...
  assert.equal(pushedURL, '?page=3&range=today&channel_name_like=demo');
  assert.equal(replacedURL, '?range=last_month');
  assert.deepEqual(calls, [
    ['pushState', null, '', '?page=3&range=today&channel_name_like=demo'],
