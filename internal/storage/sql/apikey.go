	...
	err = s.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		if s.supportsONConflict() {
			channelUpsertWithIDSQL = `
				INSERT INTO channels(id, name, url, priority, rpm_limit, max_concurrency, auth_type, codex_credential, protocol_transform_mode, enabled, scheduled_check_enabled, scheduled_check_model, cooldown_detection_rules, retry_other_keys_on_failure, created_at, updated_at)
				VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
				ON CONFLICT(id) DO UPDATE SET
					...
					codex_credential = CASE WHEN channels.auth_type = excluded.auth_type THEN excluded.codex_credential ELSE channels.codex_credential END,
			...
			channelUpsertByNameSQL = `
				INSERT INTO channels(name, url, priority, rpm_limit, max_concurrency, auth_type, codex_credential, protocol_transform_mode, enabled, scheduled_check_enabled, scheduled_check_model, cooldown_detection_rules, retry_other_keys_on_failure, created_at, updated_at)
				VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			...
			channelUpsertWithIDSQL = `
				...
				ON DUPLICATE KEY UPDATE
					...
					codex_credential = IF(auth_type = VALUES(auth_type), VALUES(codex_credential), codex_credential),
		...
		for _, cwk := range channels {
			...
			if authType == model.AuthTypeCodexOAuth && strings.TrimSpace(config.CodexCredential) == "" {
				return fmt.Errorf("import channel %s: codex_oauth channel requires a credential", config.Name)
			}
			if authType == model.AuthTypeCodexOAuth && len(cwk.APIKeys) != 0 {
				return fmt.Errorf("import channel %s: Codex channel API keys are read-only", config.Name)
			}
			if authType == model.AuthTypeAPIKey && strings.TrimSpace(config.CodexCredential) != "" {
				return fmt.Errorf("import channel %s: api_key channel cannot contain a Codex credential", config.Name)
			}
			...
			if useExplicitID {
				...
				_, err := channelStmtWithID.ExecContext(ctx,
					...
					config.RPMLimit, config.MaxConcurrency, authType, config.CodexCredential, protocolTransformMode, config.Enabled, config.ScheduledCheckEnabled, config.ScheduledCheckModel, cooldownDetectionRules, config.RetryOtherKeysOnFailure, nowUnix, nowUnix)
				if err != nil {
					return fmt.Errorf("import channel %s: %w", config.Name, err)
				}
			...
			if isUpdate && authType != model.AuthTypeCodexOAuth {
...
func (s *SQLStore) ensureAPIKeyChannelMutable(ctx context.Context, channelID int64) error {
	...
	if model.NormalizeAuthType(authType) == model.AuthTypeCodexOAuth {
		return errors.New("codex channel API keys are read-only")
	}
	return nil
}
