...
func (s *SQLStore) ListConfigs(ctx context.Context) ([]*model.Config, error) {
	...
	query := `
			SELECT c.id, c.name, c.url, c.priority, c.rpm_limit, c.max_concurrency, c.channel_type, c.websockets, c.enabled,
	...
	if modelName == "*" {
		...
		query = `
	            SELECT c.id, c.name, c.url, c.priority, c.rpm_limit, c.max_concurrency,
	                   c.channel_type, c.websockets, c.enabled, c.scheduled_check_enabled, c.scheduled_check_model,
	                   c.cooldown_until, c.cooldown_duration_ms, c.daily_cost_limit, c.cost_multiplier, c.custom_request_rules, c.cooldown_detection_rules, c.proxy_url,
	                   SUM(CASE WHEN k.id IS NOT NULL AND k.disabled = 0 THEN 1 ELSE 0 END) as key_count,
	                   c.created_at, c.updated_at
	            FROM channels c
		...
		if id == 0 {
			...
			if s.IsPostgres() {
				err := s.queryRowTx(ctx, tx, `
					INSERT INTO channels(name, url, priority, rpm_limit, max_concurrency, channel_type, websockets, enabled, scheduled_check_enabled, scheduled_check_model, daily_cost_limit, cost_multiplier, custom_request_rules, cooldown_detection_rules, proxy_url, created_at, updated_at)
					VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
					RETURNING id
				`, c.Name, c.URL, c.Priority, c.RPMLimit, c.MaxConcurrency, channelType, boolToInt(c.Websockets),
			...
			if s.supportsONConflict() {
				_, err := s.execTx(ctx, tx, `
					INSERT INTO channels(id, name, url, priority, rpm_limit, max_concurrency, channel_type, websockets, enabled, scheduled_check_enabled, scheduled_check_model, daily_cost_limit, cost_multiplier, custom_request_rules, cooldown_detection_rules, proxy_url, created_at, updated_at)
					VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
				`, id, c.Name, c.URL, c.Priority, c.RPMLimit, c.MaxConcurrency, channelType, boolToInt(c.Websockets),
					boolToInt(c.Enabled), boolToInt(c.ScheduledCheckEnabled), c.ScheduledCheckModel, c.DailyCostLimit, normalizeCostMultiplier(c.CostMultiplier), customRules, cooldownDetectionRules, c.ProxyURL, nowUnix, nowUnix)
				if err != nil {
					return err
				}
...
func (s *SQLStore) UpdateConfig(ctx context.Context, id int64, upd *model.Config) (*model.Config, error) {
	...
	err = s.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		_, err := s.execTx(ctx, tx, `
			UPDATE channels
			SET name=?, url=?, priority=?, rpm_limit=?, max_concurrency=?, channel_type=?, websockets=?, enabled=?, scheduled_check_enabled=?, scheduled_check_model=?, daily_cost_limit=?, cost_multiplier=?, custom_request_rules=?, cooldown_detection_rules=?, proxy_url=?, updated_at=?
			WHERE id=?
		`, name, url, upd.Priority, upd.RPMLimit, upd.MaxConcurrency, channelType, boolToInt(upd.Websockets),
