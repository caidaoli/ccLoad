...
func (s *SQLiteStore) AggregateRangeWithFilter(ctx context.Context, since, until time.Time, bucket time.Duration, channelType string, modelFilter string) ([]model.MetricPoint, error) {
...
func (s *SQLiteStore) GetDistinctModels(ctx context.Context, since, until time.Time) ([]string, error) {
	query := `
