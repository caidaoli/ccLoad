...
func (s *MySQLStore) migrate(ctx context.Context) error {
	...
	if _, err := s.db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS channels (
			...
			channel_type VARCHAR(64) DEFAULT 'anthropic',
			enabled TINYINT NOT NULL DEFAULT 1,
			cooldown_until BIGINT DEFAULT 0,
			cooldown_duration_ms BIGINT DEFAULT 0,
			rr_key_index INT DEFAULT 0,
	...
	if _, err := s.db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS api_keys (
			...
			key_strategy VARCHAR(32) DEFAULT 'sequential',
			cooldown_until BIGINT DEFAULT 0,
			cooldown_duration_ms BIGINT DEFAULT 0,
			created_at BIGINT NOT NULL,
			updated_at BIGINT NOT NULL,
	...
	if _, err := s.db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS auth_tokens (
			...
			stream_avg_ttfb DOUBLE DEFAULT 0.0,
			non_stream_avg_rt DOUBLE DEFAULT 0.0,
			stream_count INT DEFAULT 0,
			non_stream_count INT DEFAULT 0,
	...
	if _, err := s.db.ExecContext(ctx, `
		CREATE TABLE IF NOT EXISTS logs (
			...
			first_byte_time DOUBLE DEFAULT 0.0,
			api_key_used VARCHAR(191) DEFAULT '',
			input_tokens INT DEFAULT NULL,
			output_tokens INT DEFAULT NULL,
			cache_read_input_tokens INT DEFAULT NULL,
			cache_creation_input_tokens INT DEFAULT NULL,
