...
func fetchPreviewRelease(ctx context.Context, client *http.Client, releasesURL string) (GitHubRelease, error) {
	...
	var records []githubReleaseRecord
	...
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		if err == nil {
			return GitHubRelease{}, fmt.Errorf("decode releases: multiple JSON values")
		}
		return GitHubRelease{}, fmt.Errorf("decode releases trailing data: %w", err)
	}
	...
	for _, record := range records {
		...
		if _, ok := normalizeSemanticVersion(record.TagName); !ok {
			continue
		}
		if selected.TagName == "" || compareSemanticVersions(record.TagName, selected.TagName) > 0 {
