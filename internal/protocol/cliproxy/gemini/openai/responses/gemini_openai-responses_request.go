					...
					for _, contentItem := range partsToProcess {
						...
						default:
							if part, ok := openAIResponsesPartFromBlock(contentItem); ok {
								partJSON = part
							} else if fileID := strings.TrimSpace(contentItem.Get("file_id").String()); fileID != "" && contentType == "input_file" {
								// ccLoad: a Files API reference has no inline payload; forward it as a file URI.
								partJSON = geminiResponsesFileDataPart("application/octet-stream", fileID)
							}
						}
...
// ccLoad keeps Gemini's camelCase fileData shape, matching its other Gemini request converters.
