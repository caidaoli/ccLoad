...
func ConvertGeminiResponseToOpenAIResponses(_ context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if bytes.Equal(rawJSON, []byte("[DONE]")) {
		if st.Completed || len(st.LastRoot) == 0 {
			return [][]byte{}
		}
		finalRoot, _ := sjson.SetBytes(st.LastRoot, "candidates.0.finishReason", "STOP")
		if len(st.Usage) > 0 {
			finalRoot, _ = sjson.SetRawBytes(finalRoot, "usageMetadata", st.Usage)
		}
		return ConvertGeminiResponseToOpenAIResponses(context.Background(), modelName, originalRequestRawJSON, requestRawJSON, finalRoot, param)
	}
	...
	st.LastRoot = append(st.LastRoot[:0], root.Raw...)
