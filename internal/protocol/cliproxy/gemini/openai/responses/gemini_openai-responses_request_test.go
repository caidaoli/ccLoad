...
func TestConvertOpenAIResponsesRequestToGemini_RemoteURLDoesNotOverrideExplicitFormat(t *testing.T) {
	...
	if got := parts[0].Get("fileData.mimeType").String(); got != "audio/wav" {
		t.Errorf("audio fileData.mimeType = %q, want audio/wav. Output: %s", got, output)
	}
	if got := parts[1].Get("fileData.mimeType").String(); got != "video/mp4" {
		t.Errorf("video fileData.mimeType = %q, want video/mp4. Output: %s", got, output)
	}
}
...
func TestConvertOpenAIResponsesRequestToGemini_RemoteNestedFormatNotOverriddenByFilename(t *testing.T) {
	...
	if got := parts[0].Get("fileData.mimeType").String(); got != "video/mp4" {
		t.Errorf("video fileData.mimeType = %q, want video/mp4. Output: %s", got, output)
	}
	if got := parts[1].Get("fileData.mimeType").String(); got != "audio/wav" {
		t.Errorf("audio fileData.mimeType = %q, want audio/wav. Output: %s", got, output)
	}
	if got := parts[2].Get("fileData.mimeType").String(); got != "application/pdf" {
		t.Errorf("file fileData.mimeType = %q, want application/pdf. Output: %s", got, output)
	}
	if got := parts[3].Get("fileData.mimeType").String(); got != "image/jpeg" {
		t.Errorf("jpeg file fileData.mimeType = %q, want image/jpeg. Output: %s", got, output)
	}
	if got := parts[4].Get("fileData.mimeType").String(); got != "image/jpeg" {
		t.Errorf("image.format jpeg fileData.mimeType = %q, want image/jpeg. Output: %s", got, output)
	}
}
